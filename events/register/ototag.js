module.exports = async(client, oldUser, newUser) => {
    ...
    let tageto = client.ayar.Register.tageto;
    ...
    if (oldUser.username !== newUser.username) {
        ...
        if (!oldUser.username.includes(tag) && newUser.username.includes(tag) && !nixmember.roles.cache.has(tagrol)) {
        ...
        if (!oldUser.discriminator.includes(tageto) && newUser.discriminator.includes(tageto) && !nixmember.roles.cache.has(tagrol)) {
            nixmember.roles.add(tagrol).catch();
            if (nixmember.manageable && tag.length > 0) nixmember.setNickname(nixmember.displayName.replace(tagsız, tag)).catch();
            taglog.send({
                embed: {
                    color: "GREEN",
                    description: `${client.emojis.cache.get(client.ayar.Genel.infoemoji.yes) || "✔"} ${nixmember} **\`#${tageto}\` sembolünü kullanıcı adına alarak ekibimize katıldı.**`,
                    timestamp: new Date(),
                    footer: {
                        icon_url: newUser.displayAvatarURL({ dynamic: true }),
                        text: newUser.username
