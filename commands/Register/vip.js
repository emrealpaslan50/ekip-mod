module.exports = {
    ...
    run: async(bot, message, args) => {
        ...
        if (!member.roles.cache.has(ayar.viprol)) {
            ...
            message.react(em.yes || "✔").catch();
            message.channel.send({
                embed: {
                    color: embişx.okEmbed.color,
                    title: embişx.okEmbed.title,
                    description: `${bot.emojis.cache.get(em.yes) || "✔"} ${member} **üyenin <@&${ayar.viprol}> rolü verildi!**`,
                    timestamp: new Date(),
                    footer: {
                        icon_url: message.author.displayAvatarURL({ dynamic: true }),
                        text: embişx.noEmbed.footer
            ...
            message.channel.send({
                embed: {
                    color: embişx.okEmbed.color,
                    title: embişx.okEmbed.title,
                    description: `${bot.emojis.cache.get(em.yes) || "✔"} ${member} **üyenin <@&${ayar.viprol}> rolü alındı!**`,
                    timestamp: new Date(),
                    footer: {
                        icon_url: message.author.displayAvatarURL({ dynamic: true }),
                        text: embişx.noEmbed.footer
